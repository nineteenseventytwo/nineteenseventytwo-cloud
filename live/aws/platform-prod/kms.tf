...
# so the layer below the cluster keeps a dependency-free encryption path. That
# is correct layering, not a compromise.
...
# removes the human from that loop for about a dollar a month, and the honest
# tradeoff is written down: the cluster's secrets are unavailable if AWS KMS is
# unavailable. Provisioned now, idle until Vault lands in Phase 5, so its
# rotation policy and access are decided in the same review pass as the others
